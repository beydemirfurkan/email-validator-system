...
router.post('/:id/extend', async (req, res) => {
    try {
        ...
        if (!expiryDays || typeof expiryDays !== 'number' || expiryDays < 1 || expiryDays > 365) {
            return res.status(400).json(response_utils_1.ResponseUtils.validationError('Expiry days must be between 1 and 365'));
        }
        const existingKey = await connection_1.db.select()
            .from(schema_1.apiKeys)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.apiKeys.id, apiKeyId), (0, drizzle_orm_1.eq)(schema_1.apiKeys.userId, user.id)))
        ...
        return res.status(500).json(response_utils_1.ResponseUtils.serverError('Failed to extend API key expiry', error));
    }
});
router.get('/expired', async (req, res) => {
    try {
        ...
        const expiredKeys = await connection_1.db.select({
            ...
            .orderBy(schema_1.apiKeys.expiresAt);
        const expiredApiKeys = expiredKeys.filter(key => key.expiresAt && key.expiresAt < now);
        return res.json(response_utils_1.ResponseUtils.success({
        ...
        return res.status(500).json(response_utils_1.ResponseUtils.serverError('Failed to fetch expired API keys', error));
    }
});
router.post('/cleanup', async (req, res) => {
    try {
        ...
        const expiredKeys = await connection_1.db.select()
            .from(schema_1.apiKeys)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.apiKeys.userId, user.id), (0, drizzle_orm_1.eq)(schema_1.apiKeys.isActive, true)));
        const keysToDeactivate = expiredKeys.filter(key => key.expiresAt && key.expiresAt < now);
        ...
        return res.status(500).json(response_utils_1.ResponseUtils.serverError('Failed to cleanup expired API keys', error));
    }
});
